...
test('application modules are valid and mirrored', () => {
  const moduleFiles = [
    ...
    'src/data/must-402.js',
    'src/data/must/101.js',
    'src/data/must/102.js',
    'src/data/must/201.js',
    ...
    'src/gyn402-filtered-master-bank.js',
  ...
  assert.match(mainSource, /requestedUniversity !== activeUniversityId[\s\S]*requestedSection !== activeAcademicSection/)
...
test('account loading gate uses a compact university-neutral experience', () => {
  ...
  const mainSource = read('src/main.js')
  const style = read('src/style.css')
  const checkingPanel = html.match(/<div class="auth-gate__panel" data-auth-panel="checking">([\s\S]*?)<\/div>\s*<div class="auth-gate__panel" data-auth-panel="signed-out">/)?.[1] || ''

  assert.match(checkingPanel, /Preparing your study space…/)
  assert.match(checkingPanel, /Loading your account, university, section, and progress\./)
  assert.match(checkingPanel, /auth-gate__loader/)
  assert.match(checkingPanel, /This is taking a little longer\. Your progress is safe\./)
  assert.doesNotMatch(checkingPanel, /MUST|Misr University|Secure account sync|auth-gate__rail|Account<\/span>|Section<\/span>|Progress<\/span>/)
  ...
  assert.match(style, /\.auth-gate__loader span/)
  assert.match(style, /body\[data-auth-state="checking"\] \.onboarding-landing \.auth-gate__brandbar\s*\{\s*display:\s*none;/)
  assert.doesNotMatch(checkingPanel, /auth-gate__spinner/)
})
...
test('SUR 402-1 topic-organized past-exam MCQs are complete and wired', () => {
  ...
  const html = read('index.html')
  const must402Source = read('src/data/must-402.js')
  assert.match(html, /sur402-past-exam-mcqs\.js\?v=20260719-sur402-past-exams-v1/)
  assert.match(must402Source, /code:\s*'SUR 402-1'[\s\S]{0,220}quizTopicKey:\s*'SUR 402-1 MCQs'/)
})
...
test('MED 402-1 endocrine bank is answer-safe, grouped, and wired to the exam card', () => {
  ...
  assert.match(must402Source, /code:\s*'MED 402-1'[\s\S]{0,240}quizTopicKey:\s*'MED 402-1 MCQs'/)
})
...
test('MED 402-2 neurology bank is answer-safe, grouped, and wired to the exam card', () => {
  ...
  assert.match(must402Source, /code:\s*'MED 402-2'[\s\S]{0,260}quizTopicKey:\s*'MED 402-2 MCQs'/)
})
...
test('GYN 402 Dr Nadine VIP midterm bank is complete and wired to the exam card', () => {
  ...
  vm.runInNewContext(read('src/gyn402-filtered-master-bank.js'), context)
  ...
  const filteredMaster = quiz.sources.find((item) => item.id === 'gyn402-filtered-master-bank')
  assert.equal(filteredMaster.label, 'Filtered Master Bank')
  assert.equal(filteredMaster.mcqs.length, 484)
  assert.equal(filteredMaster.heldForReview.length, 40)
  assert.equal(new Set(filteredMaster.mcqs.map((question) => question.id)).size, 484)
  assert.ok(filteredMaster.mcqs.every((question) => question.choices.length >= 2))
  assert.ok(filteredMaster.mcqs.every((question) => Number.isInteger(question.answerIndex) && question.choices[question.answerIndex]))
  assert.equal(filteredMaster.mcqs.filter((question) => question.isJuly2026).length, 83)
  assert.deepEqual(Array.from(filteredMaster.collection.mixedSizes, (mode) => mode.size), [20, 30, 83, 484])
  assert.equal(filteredMaster.collection.groups.length, 16)
  assert.equal(filteredMaster.collection.wrongReviewId, 'gyn402-filtered-master-bank-wrong-review')
  ...
  assert.match(html, /gyn402-filtered-master-bank\.js\?v=20260730-gyn402-filtered-master-v1/)
  assert.match(must402Source, /code:\s*'GYN 402'[\s\S]{0,260}quizTopicKey:\s*'GYN 402 MCQs'/)
})
...
test('Google login is mandatory and the academic section is account-bound', () => {
  ...
  assert.match(html, /<section class="auth-gate onboarding-landing" id="section-selector" data-auth-gate/)
  ...
  assert.match(html, /id="section-selector"[\s\S]*?<section class="home-review-marquee auth-gate__reviews"/)
  assert.equal((html.match(/id="section-selector"/g) || []).length, 1)
  assert.match(html, /data-auth-section="401"/)
  assert.match(html, /data-auth-section="402"/)
  assert.match(html, /data-select-section="selector"/)
  assert.doesNotMatch(html, /data-student-switch-section|Switch university \/ section/)
  ...
  assert.match(mainSource, /if \(!user\) \{[\s\S]*setUniversitySelectionMode\('onboarding'\)[\s\S]*setAuthGateState\('needs-university'\)/)
  assert.match(mainSource, /if \(!studentProgressState\.user\) \{[\s\S]*setAuthGateState\('signed-out'\)/)
  assert.match(mainSource, /isSavedUniversity\(requestedUniversity\)[\s\S]*setAuthGateState\('needs-section'\)/)
  assert.match(mainSource, /data-auth-history/)
  ...
  assert.match(mainSource, /const fallbackUniversity = isSavedUniversity\(requestedUniversity\) \? requestedUniversity : 'must'/)
  assert.match(mainSource, /routeAuthenticatedUser\(fallbackUniversity, fallbackSection\)/)
  ...
  assert.match(mainSource, /if \(isUniversitySection\(activeUniversityId, section\)\) \{\s*showAcademicSection\(section,/)
...
test('O6U Physical Therapy is selectable, isolated, branded, and has PT-PHYS MCQs', () => {
  ...
  const style = read('src/style.css')
  const supabaseClient = read('src/supabaseClient.js')
  const migration = read('supabase/migrations/20260730011729_add_university_data_isolation.sql')
  const o6uLogo = readBytes('public/assets/o6u-university-logo.jpg')

  assert.match(html, /data-auth-panel="needs-university"/)
  assert.match(html, /data-auth-university="must"[\s\S]*data-auth-university="o6u"/)
  assert.match(profileHtml, /data-auth-university="must"[\s\S]*data-auth-university="o6u"/)
  ...
  assert.match(mainSource, /name:\s*'Physiology of Exercise'[\s\S]*Physiological Adaptation to Regular Physical Training/)
  ...
  assert.match(mainSource, /getUniversitySectionKey\s*\(/)
  assert.match(mainSource, /\$\{QUIZ_STORAGE_PREFIX\}::\$\{getProgressStorageOwnerId\(\)\}::\$\{activeUniversityId\}::\$\{activeAcademicSection\}/)
  assert.match(mainSource, /data-university-logo/)
  assert.match(mainSource, /university\.logoUrl/)

  assert.match(supabaseClient, /p_university_id:\s*universityId/)
  assert.match(supabaseClient, /user_id,university_id,section,topic_label,source_id/)
  assert.match(supabaseClient, /university_id,section,subject_code,track,topic_label/)
  assert.match(supabaseClient, /selected_university/)
  assert.match(supabaseClient, /allowed_university_id/)

  assert.match(migration, /SET selected_university = 'must'/)
  assert.match(migration, /ADD COLUMN IF NOT EXISTS university_id TEXT NOT NULL DEFAULT 'must'/)
  assert.match(migration, /PRIMARY KEY \(user_id, university_id, section, topic_label, source_id\)/)
  assert.match(migration, /UNIQUE \(university_id, section, subject_code, track, topic_label\)/)
  assert.match(migration, /preferences\.selected_university = p_university_id/)
...
test('Delta Physical Therapy is selectable and isolated with Internal Medicine only', () => {
  ...
  const style = read('src/style.css')
  const migration = read('supabase/migrations/20260730011729_add_university_data_isolation.sql')
  const deltaLogo = readBytes('public/assets/delta-university-logo.png')

  assert.match(html, /data-auth-university="must"[\s\S]*data-auth-university="o6u"[\s\S]*data-auth-university="delta"/)
  assert.match(profileHtml, /data-auth-university="delta"/)
  assert.match(html, /\/assets\/delta-university-logo\.png/)
  assert.ok(deltaLogo.length > 10_000 && deltaLogo.length < 100_000, 'Delta logo should be present and web-ready')

  assert.match(mainSource, /id:\s*'delta'[\s\S]*logoUrl:\s*'\/assets\/delta-university-logo\.png'[\s\S]*sections:\s*\['physical-therapy'\]/)
  assert.match(mainSource, /const subjectsDeltaPhysicalTherapy = \[\s*\{[\s\S]*code:\s*'DELTA-IM'[\s\S]*name:\s*'Internal Medicine'[\s\S]*topics:\s*\[\][\s\S]*\}\s*\]/)
  assert.match(mainSource, /delta:\s*\{\s*'physical-therapy':\s*\{[\s\S]*subjects:\s*subjectsDeltaPhysicalTherapy[\s\S]*midtermExamSchedule:\s*\[\]/)
  assert.match(mainSource, /delta:\s*\{\s*'physical-therapy':\s*\{\}\s*\}/)
  assert.match(mainSource, /academicSectionsByUniversity\[safeUniversityId\]\[safeSectionId\]/)
  assert.match(style, /\.auth-gate__sections\[data-university="o6u"\],\s*\.auth-gate__sections\[data-university="delta"\]/)

  assert.match(migration, /selected_university IN \('must', 'o6u', 'delta'\)/)
  assert.match(migration, /selected_university = 'delta' AND selected_section = 'physical-therapy'/)
  assert.match(migration, /university_id IN \(''must'', ''o6u'', ''delta''\)/)
  assert.doesNotMatch(migration, /\('delta', 'physical-therapy'/)
})

test('profile completion fallback is race-safe and local testing never writes cloud profile data', () => {
  const mainSource = read('src/main.js')
  const supabaseClient = read('src/supabaseClient.js')

  assert.match(supabaseClient, /const includeUniversityIsolation = universityIsolationAvailable[\s\S]*getUserPreferenceSelectFields\(includeUniversityIsolation\)/)
  assert.match(supabaseClient, /error && includeUniversityIsolation && isMissingUniversityIsolationError\(error\)/)
  assert.doesNotMatch(mainSource, /fetchUserPreference\(\)\.catch\(\(\) => null\)/)
  assert.match(mainSource, /if \(pref\) \{\s*leaderboardState\.preferences = \{ \.\.\.getDefaultUserPreferences\(\), \.\.\.pref \}/)

  assert.match(mainSource, /initialParams\.get\('local-test'\) === '1'/)
  assert.match(mainSource, /\['127\.0\.0\.1', 'localhost'\]\.includes\(window\.location\.hostname\)/)
  assert.match(mainSource, /LOCAL_TEST_SELECTION_KEY/)
  assert.match(mainSource, /localStorage\.setItem\(LOCAL_TEST_SELECTION_KEY/)
  assert.match(mainSource, /if \(isLocalTestMode\) \{[\s\S]*nickname:\s*'Local Tester'[\s\S]*profile_setup_version:\s*1/)
  assert.match(mainSource, /function initStudentSync\(\) \{\s*if \(isLocalOnboardingTestMode\)[\s\S]*if \(isLocalTestMode\)/)
})

test('university onboarding repairs preserve scope, destination, labels, and unknown schedule states', () => {
  const html = read('index.html')
  const profileHtml = read('profile.html')
  const mainSource = read('src/main.js')
  const style = read('src/style.css')

  assert.match(html, /data-auth-gate[\s\S]*aria-labelledby="auth-checking-title"/)
  assert.match(profileHtml, /data-auth-gate[\s\S]*aria-labelledby="auth-checking-title"/)
  assert.match(mainSource, /const titleIds = \{[\s\S]*checking:\s*'auth-checking-title'[\s\S]*'signed-out':\s*'auth-signin-title'[\s\S]*'needs-university':\s*'auth-university-title'[\s\S]*'needs-section':\s*'auth-section-title'/)
  assert.match(mainSource, /authGate\.setAttribute\('aria-labelledby', titleId\)/)

  assert.match(mainSource, /const remoteTrackerRefreshPromises = new Map\(\)/)
  assert.match(mainSource, /const requestedUniversity = activeUniversityId[\s\S]*remoteTrackerRefreshPromises\.get\(requestedUniversity\)/)
  assert.match(mainSource, /changed && requestedUniversity === activeUniversityId/)
  assert.doesNotMatch(mainSource, /let remoteTrackerRefreshPromise = null/)

  assert.match(mainSource, /semesterTimeline:\s*null,[\s\S]*scheduleLocation:\s*''/)
  assert.match(mainSource, /if \(!timeline\?\.start \|\| !timeline\?\.finals\) \{[\s\S]*semesterTimeline\.hidden = true/)
  assert.match(mainSource, /No confirmed exam dates have been added for this section yet\./)
  assert.doesNotMatch(mainSource, /midtermExam \? getLocalDate\(midtermExam\.date\) : new Date\('2026-07-22/)
  assert.match(mainSource, /scheduleLocation\.hidden = !activeAcademicSectionData\.scheduleLocation/)

  assert.match(mainSource, /const savedSection = leaderboardState\.preferences\?\.selected_university === 'must'[\s\S]*getDefaultSectionForUniversity\('must'\)/)
  assert.match(mainSource, /selected_university:\s*universityId,\s*selected_section:\s*savedSection/)
  assert.doesNotMatch(mainSource, /openHistoryFromOnboarding[\s\S]*?selected_section:\s*null[\s\S]*?function saveSelectedSection/)

  assert.doesNotMatch(mainSource, /requestedUniversityChange|choose-university/)
  assert.match(mainSource, /function showSelector\(options = \{\}\) \{\s*if \(studentProgressState\.user && document\.body\.dataset\.authState === 'ready'\) \{\s*openSectionSwitcher\(\)/)
  assert.match(mainSource, /function openSectionSwitcher\(\) \{[\s\S]*setUniversitySelectionMode\('switch'\)[\s\S]*setAuthGateState\('needs-university'\)/)
  assert.match(style, /\.auth-gate\[data-university-selection-mode="switch"\] \.auth-gate__brandbar,[\s\S]*#auth-university-copy\s*\{\s*display:\s*none;/)
  assert.match(mainSource, /if \(isUniversitySection\(activeUniversityId, section\)\) \{\s*showAcademicSection\(section,/)
})
...
test('student profile opens as a standalone gamified page', () => {
  ...
  assert.match(mainSource, /return mcqQuizzesByUniversity\[universityId\]\?\.\[section\] \|\| \{\}/)
...
test('mandatory profile onboarding guides each required control and cannot be skipped', () => {
  ...
  assert.match(mainSource, /redirectToRequiredProfile\(safeUniversity, safeSection\)/)
...
test('section selector is the responsive university-first onboarding landing', () => {
  const style = read('src/style.css')
  const html = read('index.html')
...
test('resource-first Drive selectors reach the shared resource renderer', async () => {
  const { mustSections } = await import('../src/data/must/sections.js')
  const mainSource = read('src/main.js')
  const functionStart = mainSource.indexOf('function getResourceItems(topic) {')
  assert.notEqual(functionStart, -1, 'getResourceItems must exist')

  let depth = 0
  let functionEnd = functionStart
  for (; functionEnd < mainSource.length; functionEnd += 1) {
    if (mainSource[functionEnd] === '{') depth += 1
    if (mainSource[functionEnd] === '}') {
      depth -= 1
      if (depth === 0) {
        functionEnd += 1
        break
  ...
  const topic = mustSections['101'].subjects[0].topics[0]
  const context = { topic, result: null, isDriveUrl: (url) => url.includes('drive.google.com') }
  vm.runInNewContext(
    `${mainSource.slice(functionStart, functionEnd)}; result = getResourceItems(topic)`,
    context
  )

  assert.equal(context.result.length, topic.driveSelector.length)
  assert.equal(context.result[0].type, 'lecture')
  assert.equal(context.result[0].url, topic.driveSelector[0].url)
})
...
test('MUST multi-class section registration, capabilities, provenance, progress isolation, and 401/402 preservation', async () => {
  ...
  assert.match(mainSource, /const driveSelectorItems = \(topic\.driveSelector \|\| \[\]\)/)
  assert.match(mainSource, /return \[\.\.\.driveSelectorItems, \.\.\.driveItems, \.\.\.otherLectureItems, \.\.\.pdfItems, \.\.\.audioItem\]/)
